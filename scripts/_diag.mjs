...
for (const city of ['as-jpn-tokyo','as-sgp-singapore','eu-rus-moscow']) {
  const gj = JSON.parse(await readFile('data/metro/'+JSON.parse(await readFile('data/metro/views/'+city+'.json','utf8')).file,'utf8'))
  let shp=null; try { shp = JSON.parse(await readFile('data/metro/llmshapes/'+city+'.orig.json','utf8')) } catch {}
  const t0 = Date.now()
