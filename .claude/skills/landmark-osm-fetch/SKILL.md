...
> 的城市 base metro features ＋ 地標合成**新的系統檔** `systems/**/{slug}-lm.geojson`
...
> route**——builder 把河流折線以**絕對 DP（偏離弦線 > 0.2 km）**簡化成站級顯著點（端點/跨河匯流共站/主要
> 轉折），產出車站 Point（`river: true`）＋路段 feature（schema 同地鐵、`route_id=river:…`、
> 色 `#00E5FF`）；骨架/格網化/爬山把它當一般線、地圖畫成一條實線（站圈以 river flag 隱藏）。
> 詳見 [[route-skeleton-connect]]。**面域（皇居/公園）仍是 landmark overlay**（`landmark_id`
> 辨識、fill＋hover＋highlight）。`data/metro/landmarks/**` 仍是**地標資料的產地**（本 skill
...
| eu-fra-paris | 河流骨架 | river-centerline | 塞納河 La Seine 1 條 **40.2 km（市區裁切）** |
