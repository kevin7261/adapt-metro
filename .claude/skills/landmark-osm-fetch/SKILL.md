...
> 的城市 base metro features ＋ 地標 features 合成**新的系統檔** `systems/**/{slug}-lm.geojson`
> （city＝「城市 + Landmark」／中文「城市＋地標」，加進 `index.json`＋`cityNamesZh.json`），
> 概念同 [[metro-city-tokyo]] 的 東京＋山手（`buildJrCombined`）。**不動 base 城市 geojson**。
> 前端 `LayerTab` 以 feature 的 `landmark_id` 把地標抽到獨立來源、用地標樣式渲染（fill／骨架線
> ＋hover popup／highlight／物件 tab）。`data/metro/landmarks/**` 仍是**地標資料的產地**（本 skill
> 的 fetch 產出、buildLandmarkCombined 的輸入），只是不再被前端直接載入。**改完地標務必重跑
> `metro:landmarkscombined`** 讓合併檔同步。
