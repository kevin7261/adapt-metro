...
- Tab 有 26 個視圖，左選單分 **7 個部份**：**原始**（格網化後＝輸入）、
  **Hill Climbing**（結果）、**直線演算法**（四個 H/V 最大化後處理，**原樣、不含
  端點拉直**：直角爬山／軸對齊／整數規劃＝本 skill 下節；**LLM 對齊**＝離線預算，
  見 [[route-llm-align]]，按鈕 badge 顯示「n輪 · 模型名」）、
  然後是每條鏈的**三步尾巴**（鏈 = 該鏈結果 → 直線縮減 → 端點拉直 → 縮減網格，
  每步一區、每條鏈一個 tab，前面的 tab 不受後面步驟影響）：
  **直線縮減**（5 個：把直線整條 H/V 平移讓「佔用欄列」更少、網格尺寸不變，
  `buildLineCompact`，見下）、
  **端點拉直**（5 個：在該鏈「直線縮減後」的結果之上做端點拉直，見下）、
  **縮減網格**（5 個，一律叫「…縮減網格」：壓縮該鏈「直線縮減→端點拉直」
  的結果，同一個 `compactGrid`）、
  最後 **直線縮減+端點拉直+縮減網格循環**（5 個，每鏈一個：在該鏈結果之上交替
  直線縮減→端點拉直→縮減網格，跑到某輪沒有點可以動為止，
  `straightenCompactLoop`，見下）。
...
- **直線縮減**（鏈第 1 步）＝`buildLineCompact(skeleton, cells, cols, rows)`：
  把**直線**——同軸共線段經 union-find 串成的連通元件，**跨相交點串接**（轉乘/
  分歧/黃色交叉點若被直線直穿就一起動）——整條剛體平移，只採納「**嚴格減少
  佔用欄數＋列數**（＝之後縮減網格能壓到的大小）」的平移，且走爬山法群集移動的
  **同一套 validShift 硬規則**（不新增交叉/遮蔽、象限與邊環繞序不變）→「網格
  越少越好、network 結構不變」；**網格尺寸不動**（收縮由鏈尾的縮減網格實現）。
  候選＝垂直於線「跳到最近的已佔用欄/列」（兩側各取 4 個，距離不限——輸入是
  稀疏格）＋兩軸 ±1/±2 滑動。tie-break：縮越多 → 邊界段 H/V 損失越小 → 位移
  小者。逐掃描迭代到沒有線可動（上限 POST_ITER_CAP=20）；每步嚴格縮小佔用
  欄列數（有下界）→ 保證收斂。stats 的 fromCols×fromRows → cols×rows 是
  **佔用**欄列數。
- **端點拉直**（鏈第 2 步）＝`buildEndpointStraighten(skeleton, cells, cols, rows)`
  （經 `iteratePost` 迭代到不動點），輸入是該鏈「直線縮減後」的佈局。
  **每個非白點（所有彩色頂點——端點/轉乘/分歧/黃色交叉；白/黑直通站不是頂點）
  ...
  直線縮減→端點拉直 都餵下游——該鏈的縮減網格（第 3 步）、RWD 底圖
  （`layer.compact` 選鏈）、llmGrid 全都壓縮這條三步鏈的輸出。畫廊（hcviews）
  每城 8 視圖（HC_VIEW_ORDER，卡片 2×4：格網化後→Hill Climbing→端點拉直
  （含直線縮減）→縮減網格 ×2 變體）。
- **直線縮減+端點拉直+縮減網格循環**＝`straightenCompactLoop(skeleton, cells,
  cols, rows)`：在該鏈結果之上每輪跑**直線縮減（自身迭代到不動點）→ 端點拉直
  （自身迭代到不動點）→ 縮減網格**，跑到某輪**兩者都沒有點可以動**為止（上限
  POST_ITER_CAP=20 輪）。壓縮會重編欄列座標——硬規則的阻擋（遮蔽/壓點）跟著變，
  可能解鎖新的拉直；拉直又空出新欄列、直線平移再解鎖更多。保證收斂：拉直嚴格
  增加 H/V（上界＝段數）且不新增佔用欄列、直線縮減嚴格減少佔用欄列（有下界）、
  格網只縮不長。回傳 stats 含 rounds/lineMoved（直線縮減）/moved（端點拉直）/
