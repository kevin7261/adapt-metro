...
- Tab 有 26 個視圖，左選單分 **8 個部份**——分組標題可點開合（手風琴，預設只
  展開含目前視圖的組；Map Adjust／RWD 的左選單共用同一套分組版面）——（端點移動/直線縮減/中位集中/循環/
...
- **跨距上限（所有移動共同守則）**：任何移動（端點移動的點移、直線縮減的線移、
  中位集中的點滑/線移）都**不得讓受影響段的兩個顏色點橫跨超過 SPAN_CAP 格**（可在樣式 tab 以「顏色點間最大跨距」滑桿設定 1–8，per-layer `layer.spanCap`、預設 2；**滑桿只改數值，按「重新計算」才套用**——D3Tab 以 appliedSpanCap 快照快取用的值，未套用前顯示提示、快取沿用舊上限。離線腳本與畫廊用預設值）
