...
- Tab 有 12 個視圖：**格網化後**（輸入）、**Hill Climbing**（結果）、
  **Hill Climbing端點拉直**（hc 鏈第二步，見下）、四個
  **H/V 最大化後處理**（直角爬山／軸對齊／整數規劃＝本 skill 下節；
  **LLM 對齊**＝離線預算，見 [[route-llm-align]]，按鈕 badge 顯示「n輪 · 模型名」）、
  **5 個縮減網格**（一律叫「…縮減網格」：Hill Climbing縮減網格＝壓縮 **端點拉直後**
  的 hc 鏈結果；直角爬山／軸對齊／整數規劃／LLM 對齊縮減網格＝壓縮各自後處理的結果，
  同一個 `compactGrid`）。
...
- **Hill Climbing端點拉直**＝`buildEndpointStraighten(skeleton, cells, cols, rows)`
  ...
  在 HC 結果（未壓縮的整數格）上，只移動**路線端點**（degree-1 彩色頂點；白/黑直通站
  不是頂點）讓其唯一小段變成 H/V。每次成功恰 +1 H/V、不會弄壞其他段（只有自己那段動）
  → 不需 net-HV 退回。每步走 makeMover 的**同一套硬規則**（不新增交叉、不壓到別的
  線/點、象限與邊環繞序不變）。方向選擇：同 route 在鄰居另一側已有 H 或 V 段就順著
  接直；否則先試位移較小的軸。**下游**：hc 縮減網格、RWD 底圖（compact='hc'）、
  llmGrid 的 'hc' 鏈都壓縮它的輸出；直角爬山/軸對齊/整數規劃/LLM 對齊仍從
  **原始 HC 結果**分支。畫廊（hcviews）每城 8 視圖（HC_VIEW_ORDER，卡片 2×4：
