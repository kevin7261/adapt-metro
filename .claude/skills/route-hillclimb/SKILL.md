...
- Tab 有 36 個視圖，左選單分 **9 個部份**：**原始**（格網化後＝輸入）、
  ...
  **端點拉直+直線縮減+中位集中+縮減網格循環**（5 個，每鏈一個：在該鏈
  結果之上交替四步，跑到某輪沒有點可以動為止，`straightenCompactLoop`，見下）、
  最後 **Step by Step**（5 個，每鏈一個：同一條四步鏈改成手動逐步——浮動面板
  按「下一步」執行一步＝目前階段的**一個單掃描**（或一次縮減網格）、
  「下一小步」＝**只做一個點/線的移動**（limit=1，三個 pass 都吃 opts.limit），
  掃不動自動換下一階段、一輪全沒動靜＝完成；「重設」回起點。
  `stepChainInit`/`stepChainNext(skeleton, state, {limit})`，state = { cells, cols,
  rows, stage, round, steps, done, lastStage, movedIds, moves, info }，D3Tab 存在
  stepState[鏈]、非 reactive；面板顯示步數＋四階段 chips（lastStage 亮起）＋
  這一步做了什麼。**前後比對**：moves = [{id, from:[c,r], to:[c,r]}]——圖上舊
  位置畫虛線空心圈、虛線軌跡連到新位置、新位置橘色實圈；小步的訊息另附座標
  （單點 (c,r)→(c,r)、整條線顯示位移向量＋成員數）。
  注意逐步的結果可能與循環 tab 略異——循環是每階段各自迭代到不動點再換，
  逐步（尤其小步）是交錯順序）。
