...
**成方護欄（形狀圖層吃成方時）**：HC 圖上的 ring＋綠折**只准剛體平移**（全體同一
`(dc,dr)`）——單點／單邊移動一律 veto，避免直線演算法／端點移動把方啃掉；半平面
合併若整塊帶著方走仍可過。機構＝`SQUARE_GUARD`＋`setFrozen({ ringIds, members })`，
`makeMover.squareOk`。members＝規定表 `stations`＋`greens`（只計 `pos` 內頂點）。
`D3Tab` 真下游才 `setFrozen`；離線 LLM 對齊／評價仍釘回成方座標。詳見 [[route-llm-shape]]。
