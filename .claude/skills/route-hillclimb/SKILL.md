...
- Tab 有 68 個視圖，左選單分 **8 個部份**——分組標題可點開合（手風琴，預設只
  展開含目前視圖的組；Map Adjust／RWD 的左選單共用同一套分組版面）——（端點移動/直線縮減/網格合併/循環/
  逐步驗證 五區**沒有 hc 鏈**——只有 直角爬山/軸對齊/整數規劃/LLM 對齊 四條鏈
  ＋七條論文鏈（筆畫法/MILP規劃/力導向/最小平方/八向格網/路徑簡化/SAT規劃，
  見 [[route-paper-align]]）各 11 tab，使用者 2026-07 裁決；RWD 底圖仍可建立在
  hc 鏈的循環上——loop 區塊的 isRWD fallback、key 'hc'）：**原始**（格網化後＝
  輸入）、**Hill Climbing**（結果）、**直線演算法**（十一個 H/V 最大化後處理，
  **原樣、不含端點移動**：直角爬山／軸對齊／整數規劃＝本 skill 下節；七條論文鏈
  ＝[[route-paper-align]]；**LLM 對齊**＝離線預算，
  ...
  **端點移動**（11 個，見下）、**直線縮減**（11 個，見下）、
  **網格合併**（11 個，見下）、
  **端點移動+直線縮減+網格合併循環**（11 個，每鏈一個：每輪＝三個演算法**各把
  ...
  最後 **逐步驗證**（11 個，每鏈一個：手動逐步——浮動面板按「下一步」
...
## H/V 最大化後處理（直角爬山/軸對齊/整數規劃＋LLM＋七條論文鏈，皆以 Hill Climbing 的 cellAfter 為輸入）
...
5. **七條論文鏈**（2026-07 新增，詳見 [[route-paper-align]]）：筆畫法／MILP規劃／
   力導向／最小平方／八向格網／路徑簡化／SAT規劃（`src/stores/paperAlign.js`，
   同契約、同 `iteratePost` 包裝；接受準則用 `countHVD`——八方向系演算法，
   45° 不算退步）。`countHV/countHVD/makeMover/applyTargets` 因此由
   hillClimb.js **export** 供 paperAlign.js 重用。

224 城實測（迭代到不動點後）：ILP 全網 H/V +47.8%、直角爬山 +42.9%、
軸對齊 +28.5%；最大迭代次數 rect 12 / ilp 8 / align 5，全數收斂；
單城最慢（NYC 直角爬山）約 1.4s。
每條鏈（hc／rect／align／ilp／llm＋七條論文鏈）各有自己的縮減網格 tab，且一律壓縮
