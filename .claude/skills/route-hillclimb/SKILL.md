...
- Tab 有 12 個視圖：**格網化後**（輸入）、**Hill Climbing**（結果）、四個
  **H/V 最大化後處理**（直角爬山／軸對齊／整數規劃＝本 skill 下節；
  **LLM 對齊**＝離線預算，見 [[route-llm-align]]，按鈕 badge 顯示「n輪 · 模型名」）、
  **5 個縮減網格**（縮減網格＝壓縮 Hill Climbing 本體；直角爬山縮減／軸對齊縮減／
  整數規劃縮減／LLM 對齊縮減＝壓縮各自後處理的結果，同一個 `compactGrid`）、
  **端點拉直**（Hill Climbing縮減 之上再疊的**新視圖**——原縮減與 RWD 底圖不變，見下節）。
  工具列在 hc/縮減網格顯示適應度 before → after（越低越好）、輪數、
  移動站數/群集數，後處理與其縮減 tab 顯示 水平垂直 before → after／總段數
  ＋移動站數＋各自的演算法統計；所有縮減 tab 另顯示 網格 cols×rows → cols'×rows'；
...
- **端點拉直**＝`buildEndpointStraighten(skeleton, cells, cols, rows)`（經 `iteratePost`
  迭代到不動點）：在 **hc 縮減結果之上**，只移動**路線端點**（degree-1 彩色頂點；
  白/黑直通站不是頂點）讓其唯一小段變成 H/V。每次成功恰 +1 H/V、不會弄壞其他段
  （只有自己那段動）→ 不需 net-HV 退回。每步走 makeMover 的**同一套硬規則**
  （不新增交叉、不壓到別的線/點、象限與邊環繞序不變）。方向選擇：同 route 在鄰居
  另一側已有 H 或 V 段就順著接直；否則先試位移較小的軸。**只是新 tab**：
  cachedCompact.hc（Hill Climbing縮減 本體、RWD 底圖、llmGrid 鏈）都不吃它的輸出；
  畫廊（hcviews）同步多 `endp-orig`/`endp-rot` 兩格（HC_VIEW_ORDER 共 8 視圖、卡片 2×4）。
...
每個後處理各有自己的縮減網格 tab（壓縮該 pass 的 cellAfter）；
**「縮減網格」本體 tab 與 RWD 仍吃 Hill Climbing 的 cellAfter**，不吃後處理結果。
