...
- Tab 有 26 個視圖，左選單分 **7 個部份**：**原始**（格網化後＝輸入）、
  ...
  **縮減網格**（5 個，一律叫「…縮減網格」：每個都壓縮該鏈
  「…→端點拉直」的結果（鏈 = 該鏈結果 → 端點拉直 → 縮減網格），
  同一個 `compactGrid`）、**直線縮減**（5 個，每鏈一個：在該鏈「拉直→縮減」
  結果之上把直線整條 H/V 平移讓網格更小，`buildLineCompact`，見下）、
  **端點拉直+縮減網格+直線縮減循環**（5 個，每鏈一個：在該鏈結果之上交替
  端點拉直→縮減網格→直線縮減，跑到某輪沒有點可以動為止，
...
- **Hill Climbing端點拉直**＝`buildEndpointStraighten(skeleton, cells, cols, rows)`
  ...
  H/V 的段接直 → 位移小者。**下游**：每條鏈（hc/rect/align/ilp/llm）的端點拉直
  都餵下游——該鏈的縮減網格、RWD 底圖（`layer.compact` 選鏈）、llmGrid
  全都壓縮「拉直後」的輸出。畫廊（hcviews）每城 8 視圖（HC_VIEW_ORDER，卡片 2×4：
  格網化後→Hill Climbing→端點拉直→縮減網格 ×2 變體）。
- **直線縮減**＝`buildLineCompact(skeleton, cells, cols, rows)`：把**直線**——
  同軸共線段經 union-find 串成的連通元件，**跨相交點串接**（轉乘/分歧/黃色
  交叉點若被直線直穿就一起動）——整條剛體平移（候選 ±1/±2、兩軸都試），
  只採納「**嚴格減少佔用欄數＋列數**（＝壓縮後網格大小）」的平移，且走爬山法
  群集移動的**同一套 validShift 硬規則**（不新增交叉/遮蔽、象限與邊環繞序不變）
  →「網格越少越好、network 結構不變」。tie-break：網格縮越多 → 邊界段 H/V 損失
  越小 → 位移小者。逐掃描迭代（每掃完 compactGrid 一次，讓下一輪 ±2 視窗搆得到
  新的合併），到沒有線可動為止（上限 POST_ITER_CAP=20）。每步嚴格縮小欄列數
  （有下界）→ 保證收斂。tab 鏈 = 該鏈結果 → 端點拉直 → 縮減網格 → 直線縮減。
- **端點拉直+縮減網格+直線縮減循環**＝`straightenCompactLoop(skeleton, cells,
  cols, rows)`：在該鏈結果之上每輪跑**端點拉直（自身迭代到不動點）→ 縮減網格 →
  直線縮減（自身迭代到不動點）**，跑到某輪**兩者都沒有點可以動**為止（上限
  POST_ITER_CAP=20 輪）。壓縮會重編欄列座標——硬規則的阻擋（遮蔽/壓點）跟著變，
  可能解鎖新的拉直；拉直又空出新欄列、直線平移再解鎖更多。保證收斂：拉直嚴格
  增加 H/V（上界＝段數）且不新增佔用欄列、直線縮減嚴格減少佔用欄列（有下界）、
  格網只縮不長。回傳 stats 含 rounds/moved（端點拉直）/lineMoved（直線縮減）/
  hvBefore→hvAfter/fromCols×fromRows→cols×rows/converged。
  直線縮減與循環都是純顯示視圖（LINE_KIND／LOOP_KIND 各 5 tab），不餵 RWD／llmGrid。
