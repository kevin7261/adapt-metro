...
- Tab 有 21 個視圖，左選單分 **6 個部份**：**原始**（格網化後＝輸入）、
  ...
  **縮減網格**（5 個，一律叫「…縮減網格」：每個都壓縮該鏈
  ...
  同一個 `compactGrid`）、**拉直縮減循環**（5 個，每鏈一個：在該鏈結果之上
  交替 端點拉直＋縮減網格，跑到某輪拉直沒有點可以動為止，
...
- **拉直縮減循環**＝`straightenCompactLoop(skeleton, cells, cols, rows)`：在該鏈
  結果之上**交替「端點拉直（自身迭代到不動點）→ 縮減網格」**，跑到某輪拉直
  **沒有點可以動**為止（上限 POST_ITER_CAP=20 輪）。壓縮不改變 H/V 關係，但會
  重編所有欄列座標——硬規則的阻擋（遮蔽/壓點）跟著變，可能解鎖新的拉直；拉直
  又可能空出新的整欄/整排給下一輪壓縮。保證收斂：每個採納的移動嚴格增加 H/V
  （上界＝段數）、格網只縮不長。回傳 `{ cellAfter, cols, rows, stats }`（stats 含
  rounds/moved/hvBefore/hvAfter/fromCols×fromRows→cols×rows/converged）。
  純顯示視圖（LOOP_KIND 5 tab），不餵 RWD／llmGrid。
