...
- Tab 有 9 個視圖：**格網化後**（輸入）、**Hill Climbing**（結果）、三個
  **H/V 最大化後處理**（直角爬山／軸對齊／整數規劃，見下節）、**4 個縮減網格**
  （縮減網格＝壓縮 Hill Climbing 本體；直角爬山縮減／軸對齊縮減／整數規劃縮減＝
  壓縮各自後處理的結果，同一個 `compactGrid`）。工具列在 hc/縮減網格顯示適應度
  before → after（越低越好）、輪數、移動站數/群集數，後處理與其縮減 tab 顯示
  水平垂直 before → after／總段數＋移動站數＋各自的演算法統計；所有縮減 tab
  另顯示 網格 cols×rows → cols'×rows'。
