...
- Tab 有 26 個視圖，左選單分 **8 個部份**——分組標題可點開合（手風琴，預設只
  ...
  **網格合併**（4 個，見下）、
  **端點移動+直線縮減+網格合併循環**（4 個，每鏈一個：每輪＝三個演算法**各把
...
- **端點移動**（鏈第 1 步）＝movewiseStage('endp')——單掃描 pass 是
  ...
  **每個非白點（所有彩色頂點——端點/轉乘/分歧/黃色交叉；白/黑直通站不是頂點）
  ...
  端點移動→直線縮減＋網格合併 ×2 變體，全部 movewise）。
...
- **網格合併**（鏈第 3 步，原「中位集中」，2026-07 重定義）＝
  movewiseStage('gather')／gridMergeSweep：把相鄰的 **row 兩兩合併、col 兩兩
  合併**——合併 r|r+1 ＝「row > r 的所有點整體上移 1 格」（半平面剛體平移，
  自帶壓縮、不留空列），col 同理左移。**不算中位點**。合法性走 validShift
  **同一套硬規則**（不壓點、不新增交叉/路線重疊、象限與邊環繞序不變＝拓撲
  不變——與端點移動/直線縮減同判準）。附帶性質：邊界段跨距只縮不增、H/V 段
  只增不減（dy=1 斜段合併後變水平；dy=1 垂直段因兩端撞格被擋）。掃描順序
  rows 由上而下、cols 由左而右，每個邊界本遍試一次（兩兩配對、合併成功即
  前進不重吃）；cursor（state.mergeCursor）讓逐步驗證的小步跨點擊延續同一遍
  （小步＝合併一個邊界）。stats 含 mergedRows/mergedCols（moved＝總合併次數）。
- **端點移動+直線縮減+網格合併循環**＝`straightenCompactLoop(skeleton,
  ...
  （直線縮減）/gatherMoved（網格合併）/hvBefore→hvAfter/
