...
- Tab 有 26 個視圖，左選單分 **7 個部份**：**原始**（格網化後＝輸入）、
  **Hill Climbing**（結果）、**直線演算法**（四個 H/V 最大化後處理，**原樣、不含
  ...
  然後是每條鏈的**三步尾巴**（鏈 = 該鏈結果 → 端點拉直 → 直線縮減 → 縮減網格，
  ...
  **縮減網格**（5 個，一律叫「…縮減網格」：壓縮該鏈「端點拉直→直線縮減」
  的結果，同一個 `compactGrid`）、
  最後 **端點拉直+直線縮減+縮減網格循環**（5 個，每鏈一個：在該鏈結果之上交替
  端點拉直→直線縮減→縮減網格，跑到某輪沒有點可以動為止，
  ...
  並在每個端點拉直 tab 的網格最底層畫一個**黃色圓標（半徑固定 24pt）**＝
  所有有色點（頂點都非白；白/黑直通站不是頂點）欄、列各自中位數的位置
  （偶數個點取平均 → 可能落在半格，像素內插）。
...
- **端點拉直+直線縮減+縮減網格循環**＝`straightenCompactLoop(skeleton, cells,
  cols, rows)`：在該鏈結果之上每輪跑**端點拉直（自身迭代到不動點）→ 直線縮減
