...
line-group 各自成站，**未列出成員（真 complex，如 Canal 東側 6/J/Z/N/Q/R/W）＝`rest` 那一站照常聚合**。
實作在 `buildGeojson.mjs` 共站合併：給每個 feat 標 `splitKeyOf=name#groupIdx`（或 `#rest`），
**union guard 擋掉「同名不同 group」的任何合併**（stop_area／距離／interchange 三 phase 都擋）。

> **⚠️ union-find 只能併不能拆**——必須在合併**發生前**用 guard 擋（曾錯在事後才拆、被 stop_area
> phase 先併走 → g0/g1 同 root 拆不開）。guard 只對「有 splitKeyOf 的成員」生效，其他城市/站不受影響。

已裁決（使用者 2026-07 逐站回報）：Canal St [1/2/3 | A/C/E | rest東側複合體]、Rector St [1/2/3 | N/Q/R/W]。
**待確認**：WTC 一帶（Cortlandt Street 現併成 1/2/3/A/C/E/N/Q/R/W、WTC Cortlandt、Chambers）互連
（WTC Oculus）複雜，哪些線真轉乘需逐一裁決後補進 `split`。
