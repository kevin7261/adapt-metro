...
| `interchange_rule` | error | **interchange ⇔ 通過次數 ≥2**——`pass_count`（路段化前逐 route 計，每 route 通過同站計一次）≥2 ⇔ `is_interchange`。**不同 route 各通過一次都算換乘**：支線分歧（七張＝綠線主線＋小碧潭支線）、同系統獨立營運線交會（濱海沙崙＝綠山線＋藍海線）皆為 interchange，即使同 ref。**同一條 route** 環線閉合/折返經過同站兩次不算（passCount 已將閉合點減 1）。前端站點紅色用此 `is_interchange` 欄位 |
