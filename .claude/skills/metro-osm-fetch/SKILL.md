...
1. `scripts/fetchOceania.mjs`（`npm run metro:fetchoceania`）：墨爾本（Metro Trains 16 線
   ＋Yarra Trams 24 路線，**分成兩個系統檔**）、布里斯本、伯斯、阿德萊德（含電車）、
   奧克蘭、威靈頓。
...
**電車路線變體一律合併（使用者裁決 2026-07-23「tram 同路線就要合併，不要分開畫；車站
沒停車就是 pass」）**：`route=tram` 的群組只取最長變體當該路線唯一的線（`dedupeSeqs(...).slice(0,1)`），
不套用一般的「分支＝獨立路線」。墨爾本電車的 12d／57a／57d／75d 等區間車若各自成線，
24 條路線會畫成 35 條同色重疊線；區間車只停部分站＝pass。
