...
1. `scripts/fetchOceania.mjs`（`npm run metro:fetchoceania`）：墨爾本（Metro Trains
   ＋Yarra Trams，**分成兩個系統檔**；train 剔除 Cross City／City Circle 以免污染
   SHM／WIL）、布里斯本、伯斯、阿德萊德（含電車）、奧克蘭、威靈頓。
...
**電車同路線畫共線（使用者裁決 2026-07-23「同路線就要畫共線；沒停站才用 pass」）**：
`route=tram` 同 ref 的變體併成**一個** route（不套用「分支＝獨立路線」）。主線＝最長變體；
其餘只保留分岔 spur（接軌站＋新站），避免反向／區間整段重走脹站序。停靠＝聯集。墨爾本
12d／57a／57d／75d 若各自成線會把 24 條畫成 35 條同色重疊。**pass 只用在幾何行經但不停靠**
（快車／`express_passthrough`）；區間車沒跑到的外段不是 pass、也不另畫線。

**同城重疊路網分檔（`SPLIT_SIBLINGS`）**：同一座城市的兩個路網各自成檔時（目前只有墨爾本
鐵路 ↔ 電車），站點的「跨城轉乘複製」會把電車停留場留在鐵路桶裡，接著被共站合併／snap
吸進鐵路線。故在共站合併前，依明列的姊妹桶把走錯桶的站複本剔除：有 route 成員歸屬的看
歸屬、沒有歸屬的（`lines` 只是 900 m 鄰近猜測）看站自帶 network 被 `NETWORK_CITY` 釘到哪一城。
只作用於明列的姊妹桶，其它城市的跨城轉乘複本行為不變。
...
（如紐約 23rd St 分屬 6 條線各自獨立）。**墨爾本電車**關掉同名合併（`SKIP_SAMENAME`／
`TRAM_JUNCTION_MERGE`）：共站以官方路網圖的同位置為準（同 Stop 號近距／異線路口近距），
同名≠共站、異名≠非共站——見 [[metro-cities]]。另：line 頂點吸附對「合併前成員點」找最近再
