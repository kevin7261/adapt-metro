---
name: route-llm-grid
description: LLM 調整（RWD Maps「AI 改網格長寬」）——不用 API key，由 Claude Code 的模型直接依使用者的一句話（例「把市中心那幾欄拉開」）推理路網網格每個 X 欄／Y 列區間的顯示權重：export 讀欄列脈絡 → 推理權重 → apply 驗證並存到 data/metro/llmgrids，網頁的「LLM調整」tab 只載入結果並在新像素座標重畫 H/V/45°。當使用者要求跑/重跑某城市的 LLM 調整、用自然語言改網格大小、產生 llmgrid、或問 LLM調整 tab 為何顯示「尚未產生」時使用。畫線規則見 [[route-rwd-draw]]。
---

# LLM 調整：AI 改網格長寬 (route-llm-grid)
...
- 網頁端：RWD 視圖的第三個 tab「**LLM調整**」——內容同 RWD 路網（同一套
  H/V/45° 畫線、黑點、自動隱藏白點），只是欄寬列高由本結果的區間權重決定
  （動畫過渡：內插格線位置、每幀 fast 重畫，最後一幀完整品質）。
- **按鈕觸發（不自動開始）**：沒有結果時畫面上有輸入框＋「開始 LLM 調整」鈕
  ——按下去 POST `/llm-grid/run`（vite dev plugin spawn **headless
  `claude -p`** 跑本 skill），頁面輪詢 `/llm-grid/status`、即時串流回傳文字，
  跑完自動載入。GH Pages 沒有 dev server → 按鈕會回報需要本機 `npm run dev`。
  測試替身：`LLM_GRID_CMD` 環境變數可換掉 `claude` 指令。
- **右側面板「LLM調整」tab**（StylePanel，viewKind='rwd' 時常駐）：自然語言
  輸入框＋執行鈕；有結果時顯示 模型／使用者指示／note／欄列權重摘要。
