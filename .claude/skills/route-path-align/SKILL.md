...
2. **C-directed 最少 link（Definition 1）**：C = 8 方向、每點 ε-圓（ε = 段長
   中位數夾在 [1, WINDOW]——論文建議「相鄰站均距 1–2 倍」起試；固定點 ε 收縮成
   1e-6＝「恰過該點」）。
   `reach(i, c)`＝從點 i 起、方向 c 的單一 link 能**依序刺穿**到的最遠索引：
   offset 可行區間收縮＋取中點、沿 link 參數單調驗證（貪婪）。實作採 O(|C|²·n²)
   直觀版（論文自己說可以先寫直觀版）。
3. **BFS 分層求最少 link 數**：狀態＝(點索引, 本 link 方向)——記方向才能施加 §3
   的兩個品質擴充：**最大彎角 α = 90°**（相鄰 link 的方向環狀差 > 2 ＝內角 < 90°
   的銳角彎，直接跳過；差 0 是同向、不算新 link）與**最小 link 長**（每個 link
   至少往前吃一個輸入點，零長 link 不可能出現）。ε/彎角限制蓋不住（罕見）該鏈放棄。
4. **投影**：回溯 links，每 link 的 offset 取兩端中點（固定端以其為準），中間
   頂點**垂直投影**到 link 上；每條鏈**當場套用**（`makeApplier`），後續路線看到
   的是已定案的佈局（漸進式）。
...
ε/方向集/最大彎角/排序變動同步本檔與 `paperAlign.js`。**bump VIEWS_VERSION**。
