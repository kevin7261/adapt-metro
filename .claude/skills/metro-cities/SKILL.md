...
| 墨爾本電車 | tram | `NETWORK_CITY` PTV - Metropolitan Trams | `oc-aus-melbourne-tram`＝Yarra Trams **24 路線**（同 ref 變體併成一條共線；停靠＝stop 聯集；**pass 只用在行經不停**）。**共站以官方路網圖為準，不以同名／異名**：關掉同名≤800 m；同 Stop 號（含 D1）&lt;110 m；&lt;12 m 強制；&lt;30 m 異線；**30–50 m 異線且街名不同**才併（Windsor Chapel↔Dandenong 48 m；同街異號如 Bourke Mall 3↔5 不併）。改 geojson 後必須 `npm run metro:views -- oc-aus-melbourne-tram` 否則 Map Adjust 仍是舊檔。官方圖：[Melbourne tram network](https://edge.sitecorecloud.io/stategovernc45d-cftw-production-c9ca/media/Project/TransportWebsite/TransportWebsite/News-and-resources/Maps/tram/Melbourne-tram-network-map.pdf) |
