...
| 墨爾本電車 | tram | `NETWORK_CITY` PTV - Metropolitan Trams | `oc-aus-melbourne-tram`＝Yarra Trams **24 路線**（含 6／75／109 等現行號；同號區間車只取最長變體、其餘 pass）。兩桶重疊站複本由 `SPLIT_SIBLINGS` 清掉。官方圖：[Melbourne tram network](https://edge.sitecorecloud.io/stategovernc45d-cftw-production-c9ca/media/Project/TransportWebsite/TransportWebsite/News-and-resources/Maps/tram/Melbourne-tram-network-map.pdf)（含共站／共線示意） |
