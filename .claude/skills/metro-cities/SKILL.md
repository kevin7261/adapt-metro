...
| 城市 | 模式 | 綁城方式 | 裁決重點 |
|---|---|---|---|
| 墨爾本 | train | `NETWORK_CITY` PTV - Metropolitan Trains | **鐵路與電車分成兩個系統檔**（使用者 2026-07-23）：`oc-aus-melbourne`＝Metro Trains。Metro Tunnel 貫通運營的箭頭 ref（`WGS=>CBE`）不屬任何 master，補丁設 `ref=CBE` 併回 Cranbourne 線。**剔除 Cross City（Williamstown⇄Sandringham）與 train City Circle**——會污染 SHM／WIL 站序；共站／共線依幹線共線與站合併處理。官方圖：[Victorian train network](https://edge.sitecorecloud.io/stategovernc45d-cftw-production-c9ca/media/Project/TransportWebsite/Forms/victorian-train-network-map.pdf) |
| 墨爾本電車 | tram | `NETWORK_CITY` PTV - Metropolitan Trams | `oc-aus-melbourne-tram`＝Yarra Trams **24 路線**（同 ref 變體併成一條共線；停靠＝stop 聯集；**pass 只用在行經不停**）。**共站以官方路網圖為準，不以同名／異名**：關掉同名≤800 m；同 Stop 號（含 D1）&lt;110 m（不可放大——否則 CBD 同號串進 Flinders）；&lt;12 m 強制；&lt;30 m 異線；**30–90 m 異線且街名不同**才併（路口轉乘，含 Swanston↔Bourke Mall；同街異號如 Bourke Mall 3↔5 不併）；具名樞紐一般 &lt;200 m，**Melbourne Central 兩側 &lt;300 m**。跳站捷徑弦 passMinKm＝0.2 km；snap 優先本線 stop 成員。改 geojson 後必須 `npm run metro:views -- oc-aus-melbourne-tram`。官方圖：[Melbourne tram network](https://edge.sitecorecloud.io/stategovernc45d-cftw-production-c9ca/media/Project/TransportWebsite/TransportWebsite/News-and-resources/Maps/tram/Melbourne-tram-network-map.pdf) |
