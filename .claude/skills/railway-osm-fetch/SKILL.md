...
台灣（高鐵 1 線/12 站、台鐵 15 線/241 站）、日本（新幹線 8 線/100 站、JR 一般 ~190 線/4370 站）、
中國（**只高鐵路網** 203 線/1982 站，無一般檔）、韓國（KTX 1 線/9 站、Korail 36 線/403 站）。
...
# relations — 高鐵路線關聯（route=railway 跑在高鐵軌上者）＋其停站成員（供 §5b 建高鐵線）
[out:json][timeout:600];
area["ISO3166-1"="TW"][admin_level=2]->.a;
way["railway"="rail"]["highspeed"="yes"](area.a)->.hsw;
rel(bw.hsw)["route"="railway"]->.hsrel;
.hsrel out body;                                        # 關聯 tags＋有序 member refs
node(r.hsrel)["railway"~"^(station|halt|stop)$"]; out;  # 停站節點：座標＋名（不需軌道幾何，直線連相鄰站）
```

原始回應**一國一檔**快取 `data/railway/_cache/rw_{cc}.json`（含 `trackElements/stationElements/relElements`）。
**增量補抓**：快取已有 track/station 但缺 relElements 時，`railway:fetch` 只抓 relations 併回，不重抓龐大軌道
...
3. **連通性來自真實軌道圖，不是線名（核心，使用者：花蓮-台東/台中-彰化/屏東-台東 都不可中斷）**：
   ...
   **軌道走查只建「一般國鐵」，一律 `if (e0.hs) continue` 跳過高鐵段**（走查碰到高鐵軌也 break）。
3b. **高鐵（high_speed）改由 OSM `route=railway` 路線關聯的「有序停站成員」建（核心，使用者：同一路線
   一定串接）**：軌道走查會把**高架橋下的地方站**誤抓進來（東海道新幹線高架 ← 東急武蔵小杉 32m），
   且 `highspeed=yes` 站標太稀疏（中國幾乎沒有）→ 幾何/標籤都不可靠。關聯的**停站成員才是真停站**
   （含 tag-less 的 小田原/名古屋，排除高架 passover）。做法（`buildRailwayGeojson.mjs` §5b）：
   ① `isHsrRelation`：關聯的**member way ≥50% 是 highspeed** 才算高鐵關聯（擋掉只共用一條高鐵 way 的
      一般線，如 上越線 ← ガーラ湯沢 上越新幹線支線）；② 取 member 中 `railway=station/halt/stop` 的
      節點（排除 `线路所/信号場` 號誌站），`nodeToStation` 對應到合併站（對不到就**建新站**，不漏停站）；
   ③ **`netToStations` 補站**：`network` 標為某高鐵線且**帶高鐵標記**的站補進（品川 network=東海道新幹線
      → 補回；但 network 無高鐵標記者不補，否則 上越線 36 站全灌進來）；④ **同線名跨多關聯先合併**再
      **NN 排序、無距離上限連相鄰**（關聯權威 → 保證整條 1 段串接）；⑤ `nameFor` 去 `^JR` 前綴讓
      `JR東北本線≡東北本線` 併。**無高鐵關聯的國家（韓國 KTX：OSM 只關聯一條隧道＋一號線地鐵）退回
      fallback**：把所有 `hsr` 旗標站 NN 串成一條 `高速鐵路`（多線會併成一條，不完美但有呈現）。
   實測：東海道新幹線 17 站 1 段（＝wiki）、京沪高铁 23 站 1 段、日本 8 條新幹線、中國 203 條高鐵線。
4. **class 由「邊怎麼來的」決定，不看線名**：`edgeClass = rec.hs ? high_speed : conventional`——關聯建的邊＝
   高鐵、走查建的邊＝一般。**所以帶高鐵段的一般線（韓國 Honam Line）整條留在一般國鐵**，不因 `clsOfLine`
   看到一條高鐵 way 就整條跳去高鐵。`routeMeta` 的 rail_class/顏色一律取所在檔的 class。
...
- **高鐵靠 OSM route=railway 關聯的有序停站**（見 §3b）：東海道新幹線 17 站/京沪高铁 23 站皆 1 段串接、
  對得上 wiki。少數線 2 段是**與並行高鐵線共用主幹**（北陸↔上越新幹線共 東京-高崎）在 feature 層被拆給
  另一線，站序仍完整、同色，屬 label 拆分非拓撲斷。中國高鐵 14/203 線多段（分省多關聯或長線分歧）。
- **韓國高鐵不完美**：OSM 對 KTX 京釜/湖南高速線**沒有可用的 route=railway 關聯**（只關聯到一條隧道＋
  一號線地鐵），退回 fallback 把 `hsr` 旗標站串成**單一條**「高速鐵路」（京釜＋湖南被併）＝有呈現但不分線；
  要修須等 OSM 補關聯，或改用 route=train / 逐線 wiki 站表 override。
- **一般國鐵沿用台灣式軌道走查**：同一線名在交會換名處被切成多段 label（台灣 縱貫線×4、日本一般 40%、
  韓國一般 38%）＝**顏色連續、拓撲不斷**（使用者：台灣是正確的）。已加 `^JR` 前綴正規化（`JR東北本線≡
  東北本線`）減少變體；要更嚴謹可加逐國線名正規化表。
...
└── systems/{洲全名}/{國slug}/{cc}-{hsr|rail}.geojson   # 一國拆兩檔！as-jpn-hsr.geojson（高鐵）＋
                                                        # as-jpn-rail.geojson（一般國鐵）；中國只有 -hsr
```
