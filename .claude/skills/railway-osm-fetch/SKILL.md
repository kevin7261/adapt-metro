...
台灣需排除**台糖五分車**（觀光糖鐵，非國鐵客運）：`as-twn_exclude.json` 已建。
...
2. **車站合併**（union-find，同 metro/highway 共站）：同正規化名 ≤2.5km 或純距離 <150m 併為一站，
   代表座標取質心、名取眾數。**同城異址的高鐵站與台鐵站（新竹/桃園/嘉義… 相距數 km）不會併**
   （>2.5km）＝正確保留為不同站。併站時若成員含**高鐵營運商節點**則標記 `hsr`（供步驟 4）。
3. **逐線把車站沿軌道排序（核心，取代 BFS）**：對每條線——把該線 ways 貪婪串成 spine 折線；
   線上車站（距該線軌道 vertex ≤ `LINE_SNAP` 400m）投影到 spine 取弧長位置、依位置排序、
   **相鄰兩站連一條邊**（>`MAX_EDGE_M` 35km 視為斷口不連）。**平行上下行線與雙線**因無序對
   `ekey(a,b)` 去重而自動收斂成一條；交會點只在「兩線共站」處出現 → degree 乾淨（不會像 BFS
   在調車場/平行線 mesh 中把一站連到十幾站）。
...
6. **路段化（同 metro/highway）**：全域邊去重後，「行經線集合相同」的邊串成一個路段 feature
   （座標連續長折線）；`routes[]` 列該段所有線。只輸出出現在某段上的車站（orphan-drop）。
...
- **高鐵完整度受 OSM 幾何/站點偏移影響**：台灣實測高鐵 9/12 站（台北/桃園/台中因隧道幾何稀疏
  或站點偏移未吸附）。可加**逐國高鐵站 override** 補齊；一般鐵路網完整正確。
