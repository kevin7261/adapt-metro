...
3. **逐線把車站排成一條（核心，取代 BFS）**：對每條線——① 成員判定：距該線軌道 vertex
   ≤ `LINE_SNAP` 400m 的車站屬於此線；② 排序＝**最近鄰路徑（NN）從直徑端點出發**（比 1-D
   spine 投影穩健——長/彎/分歧線如縱貫線用 spine 投影會把遠站投到同弧長而錯亂）；③ 相鄰兩站
   連邊（>`MAX_EDGE_M` 40km 視為斷口）。**平行上下行/雙線**因站對 `ekey(a,b)` 去重自動收斂成
   一條；交會點只在「兩線共站」處出現 → degree 乾淨（縱貫線實測 degree 幾乎全 2，無 BFS 的
   調車場 mesh 爆度數問題）。
...
6. **一線一 feature（跟 metro 一樣，使用者：同路線要同一條、不是一段一段）**：每條線輸出
   **一個** line feature＝該線車站依序連成的**單一連續折線**（`routes:[該線]`、`route_count:1`）；
   **不做 highway 式的共線走廊切段**（那會把縱貫線切成幾十段）。重疊的線各自畫整條、疊著畫。
   **斷口門檻 per-class**（`capFor`）：一般線 `MAX_EDGE_M` 30km、高鐵 `MAX_EDGE_HSR_M` 70km——
   因高鐵站本就相距 30–60km（連續專用軌，需高門檻才不斷），而一般線的大缺口＝**該線根本
   沒軌**（縱貫線 竹南↔彰化 那段是山線 台中線／海線 海岸線，縱貫線名只在兩端 → **正確拆 2 段**，
   不可畫成一條直跨 60km 的假線，使用者指正）。實測台灣：縱貫線 2 段（山/海線）、其餘 16 條與
   高鐵各為單一連續線；最長一般線邊 19km（南迴實缺口）、最長高鐵邊 60km。
   **高鐵線名去重**：高鐵軌道可能有名稱變體（台灣高速鐵路／高速鐵路），一個名字是另一個的子字串
   且同屬高鐵軌時併為一條（`hsNames` substring 合併）；相異名的多條新幹線（東海道 vs 山陽，互非
   子字串）不受影響 → 日本多條新幹線仍各自獨立。
