...
1. **ldeg 與邊排序（§4.1 / STEP 3）**：`ldeg(v)` ＝鄰接段的不重複路線數總和；
   邊序照論文的 UNPROCESSED → DANGLING → PROCESSED 排程：從 ldeg 最高的站起，
   每次取 DANGLING 中 ldeg 最高者、把它未處理的鄰居（依 ldeg 降冪）逐一接出
   ——複雜轉乘樞紐先佔位、由內往外長。
2. **逐邊定案**：每邊的未定案端點取原位 Chebyshev 半徑 `R=WINDOW` 內的**空格**
   候選（已定案端點只剩定案格），對每組 (A,B) 計成本：
   - **位移懲罰**（STEP 4）：與原位的歐氏距離 × `(c_h + c_m) = 1.5`；
   - **格網移動成本**：走過的跳數 × `c_h − a`（H/V = 0、對角 = 0.5，成本偏移
     `a = c_45 − c_135 = 1`，論文 STEP 2 的輕微 H/V 偏好）；弦若不是正八方向，
     最省也要靠兩個 135° 折繞，加 `2 × c_135`；
   - **站上線彎（§4.4）**：與該頂點上已定案同路線段的彎折 `BEND[Δ]`，Δ =兩出向
     環狀差：`Δ=4` 直行 `c_180=0`、`Δ=3` → `c_135=1`、`Δ=2` → `c_90=1.5`、
     `Δ=1` → `c_45=2`、`Δ=0`（兩段同向重疊）不允許。
...
成本參數（c_45/c_90/c_135/c_h/c_m/a）/半徑/排序變動同步本檔與 `paperAlign.js`。**bump VIEWS_VERSION**。
