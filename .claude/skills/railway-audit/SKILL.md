...
- **日本一般國鐵 warn（我們 < wiki）已大幅收斂**：`trackOrder` 逐元件串接修好後（見 [[railway-osm-fetch]] §3），
  山陰本線 58→158/161、予讃線 52→88/97、奥羽本線 41→104/103、琵琶湖線 補回 23/23——先前缺的站是「線自己的
  軌道在 18m grid 裂成多元件、只取最大元件」漏掉、被丟進 `一般鐵路` connector（斷開/抓錯）。剩餘 warn（東武東上線
  9/39、総武本線 22/49…）多為 OSM `route=railway` 關聯／軌道本身停站不全，定向重抓可再補。
- **日本拆 JR 六社的邊界退化已解**：琵琶湖線京都端誤分 JR東海、雁ヶ沢/吾妻峡八ッ場——前者經 §3 逐元件串接
  自動修正（線完整串在 west 內）；後者是八ッ場壩淹沒的 2014 舊線殘站（距現役軌道 ~980m），拆分後正確剔除。
- **迷你新幹線微幅多抓**：山形/秋田/北陸新幹線 走一般線共用軌，`trackOrder` 串接會沿共用軌多收幾站
  （山形新幹線 我16）——屬 mini-shinkansen 共軌固有，非重覆/斷開。
