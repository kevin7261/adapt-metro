...
| `no_adjacent_dup_names` | error | **相鄰同名站未併**（宜蘭交流道 6.1km 案例）——不靠 wiki 表也抓得到；無名佔位「交流道」與「里程差 ≥1km 的同名不同出口」（台61 林口 14/16）豁免 |
| `wiki_order` | error | （台灣）各路站序與 wiki 里程表一致（吻合站的 km 單調） |
| `wiki_geometry` | error | （台灣）相鄰站直線距離 ≤ 里程差×1.3+1.2——直線比路長＝接錯線；寬放係數容忍匝道端 vs 樁位差 |
| `wiki_missing` | warn | （台灣）bbox 內 wiki 有、我們缺的交流道（OSM 上游缺節點/節點只在匝道上） |
| `wiki_extra` | warn | （台灣）我們有名字但不在 wiki 表上（上游異名如「安樂端」=wiki 基金交流道、wiki 頁漏列） |
