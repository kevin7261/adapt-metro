...
const SEED_MOVES = JSON.parse(fs2.readFileSync(process.argv[4]||'.scratch/seed_funabori.json','utf8')).moves
let bestRun = null
for (let run = 0; run < RESTARTS; run++) {
  ...
  for (const [i,P] of Object.entries(SEED_MOVES)) { const id = idsSorted[+i]; if (M.validMove(id,P)) M.applyMove(id,P); else console.log('seed rejected', i, nameById.get(id)) }
  let cur = metrics(pos)
  const applied = []
  const done = new Set()
...
const moves = {}
for (const [i,P] of Object.entries(SEED_MOVES)) moves[i]=P
for (const [id, P, from] of bestRun.applied) {
  moves[idx.get(id)] = P
  console.log(`  i${idx.get(id)} ${nameById.get(id)} (${from}) -> (${P})`)
}
writeFileSync(process.argv[5]||'.scratch/moves3.json', JSON.stringify(moves, null, 1))
