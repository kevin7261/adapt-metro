...
1. **照指定形狀**：規定路段嵌成正方形（四條 H/V 邊）。**一定要算出來**——不成方不得靜默退回。
2. **拓撲優先（D1）**：不當交叉與點撞格不得比輸入更差；邊環繞序盡量不變。鐵律版先求；失敗則強制成方，必要時加 **綠色控制點** 讓路線轉折消交叉後交付。
...
| 拓撲 | **優先鐵律**：不當交叉 ≤ 輸入；點周圍邊環繞序不變；無撞格 |
| 失敗策略 | 鐵律版失敗 → **§6F 強制成方**；必要時插入綠色控制點（`shape-g*`）轉折消交叉後仍交付（禁止只退回輸入） |
...
| 新交叉退回＋懲罰 | LS 輪回退；§6 先鐵律，否則強制成方＋綠控 |
