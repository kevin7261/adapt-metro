...
| 直線演算法 | 直角爬山／軸對齊／整數規劃／LLM 對齊＋七條論文鏈（筆畫法／MILP規劃／力導向／最小平方／八向格網／路徑簡化／SAT規劃） | 十一種 H/V 最大化後處理（＝十一條「鏈」），互相獨立可比較，共用同一套硬規則，各自迭代到不動點（skills `route-rect-polish` / `route-axis-align` / `route-axis-ilp` / `route-llm-align`；論文鏈總機構 `route-paper-align`＋各自的 `route-{stroke,milp,force,lsq,octi,path,sat}-align`，實作 `src/stores/paperAlign.js`） |
| 端點移動 | 每條鏈一個 tab（共 11 個） | 三步鏈第 1 步（skill `route-endpoint-move`） |
| 直線縮減 | 每條鏈一個 tab（共 11 個） | 三步鏈第 2 步（skill `route-line-compact`） |
| 網格合併 | 每條鏈一個 tab（共 11 個） | 三步鏈第 3 步（skill `route-grid-merge`） |
| 端點移動+直線縮減+網格合併循環 | 每條鏈一個 tab（共 11 個） | 三個演算法各掃整個 network 一遍、輪替到全靜止（skill `route-movewise-loop`） |
| 逐步驗證 | 每條鏈一個 tab（共 11 個） | 一鍵一步看演算法怎麼收斂（skill `route-step-verify`） |
...
直線演算法原四條鏈的比較基準（迭代到不動點後）：ILP 全網 H/V +47.8%、
直角爬山 +42.9%、軸對齊 +28.5%，全數收斂。七條論文鏈（2026-07 新增，
...
- 視圖組成：`views/` 8 個（原始/旋轉/骨架化/格網化前後 ×2）、`hcviews/` 26 個
  （每變體：格網化後 → Hill Climbing → 11 條鏈的循環結果，×2 變體）、
  `rwdviews/` 44 個（11 條鏈 ×〔循環結果｜RWD 路網〕×2 變體）。
- **`_fp` 增量重算**：`metro:views` 重跑時指紋沒變的城市直接沿用舊檔，只重算資料變了的城市；
  **改了任何演算法**要把 `buildViews.mjs` 的 `VIEWS_VERSION` +1 強制全量重算
  （版本註解就是演算法變更日誌，目前 35；RWD 佈線器另有 `RWD_ROUTER_REV` 修訂字串）。
...
| LLM 比較（RWD Maps） | 網頁按鈕 → `/llm-compare/run` | skill `route-llm-compare`：唯讀評審——一次比較原始與旋轉變體的直角爬山/軸對齊/整數規劃/七條論文鏈/LLM 對齊（最多 22 個結果），依方正/直線/轉折/平衡/硬失敗選出全體最佳、原始最佳、旋轉最佳並逐一列優缺點 → 寫 `llmcompares/` |
...
編號 1/3–8 的七篇（2026-07 使用者裁決）都以「整數格短距離後處理」形式實作成
Straighten 的**論文直線鏈**（`src/stores/paperAlign.js`，總機構 skill
`route-paper-align`），與原四鏈共用同一套硬規則與全部下游功能。
