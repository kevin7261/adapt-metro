...
| 直線演算法 | 論文①〜⑨（①筆畫法／②直角爬山／③MILP規劃／④力導向／⑤最小平方／⑥八向格網／⑦路徑簡化／⑧SAT規劃／⑨Shape-Guided）＋LLM 對齊 | 十種 H/V 最大化後處理（＝十條「鏈」），名稱與 `data/thesis/<n>_*_演算法說明.md` 一一對應；互相獨立可比較，共用同一套硬規則，各自迭代到不動點（總機構 `route-paper-align`＋各自的 `route-{stroke,milp,force,lsq,octi,path,sat,shape}-align` 與 `route-rect-polish`／`route-llm-align`，實作 `src/stores/paperAlign.js`） |
...
| 9 | Batik et al. 2022, Shape-Guided Mixed Metro Map Layout (Pacific Graphics) | **直線鏈「⑨Shape-Guided」**（`route-shape-align`；自動選路＋內建形狀，不適合略過） |
