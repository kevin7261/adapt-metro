...
實作在 `src/stores/hillClimb.js`＋`src/stores/paperAlign.js`（皆純函式）；
D3 分頁左選單分 **8 個部份、56 個視圖**：
...
| 直線演算法 | 論文①〜⑧（①筆畫法／②直角爬山／③MILP規劃／④力導向／⑤最小平方／⑥八向格網／⑦路徑簡化／⑧SAT規劃）＋LLM 對齊 | 九種 H/V 最大化後處理（＝九條「鏈」），名稱與 `data/thesis/<n>_*_演算法說明.md` 一一對應；互相獨立可比較，共用同一套硬規則，各自迭代到不動點（總機構 `route-paper-align`＋各自的 `route-{stroke,milp,force,lsq,octi,path,sat}-align` 與 `route-rect-polish`／`route-llm-align`，實作 `src/stores/paperAlign.js`） |
| Shape-Guided | 對①〜⑧循環結果貼形 | 掛在「循環」與「逐步驗證」之間（`route-shape-align`）；自動選路＋內建方形，不適合略過；**不是**直線演算法第 9 條 |
| 端點移動 | 每條鏈一個 tab（共 9 個） | 三步鏈第 1 步（skill `route-endpoint-move`） |
| 直線縮減 | 每條鏈一個 tab（共 9 個） | 三步鏈第 2 步（skill `route-line-compact`） |
| 網格合併 | 每條鏈一個 tab（共 9 個） | 三步鏈第 3 步（skill `route-grid-merge`） |
| 端點移動+直線縮減+網格合併循環 | 每條鏈一個 tab（共 9 個） | 三個演算法各掃整個 network 一遍、輪替到全靜止（skill `route-movewise-loop`） |
| 逐步驗證 | 每條鏈一個 tab（共 9 個） | 一鍵一步看演算法怎麼收斂（skill `route-step-verify`） |
...
| 步 | 演算法 | 移動 | 採納條件 | 收斂單調量 |
|---|---|---|---|---|
| 1 | **端點移動** | 每個非白點移 1 格（八方向含 45°） | H/V 淨增優先；否則挑入射段縮最短者。bendsPaid：既有直段只有同路線同步被拉直才准折彎 | (−H/V 數, 總長) 字典序嚴格下降 |
| 2 | **直線縮減** | 跨相交點串接的整條直線八方向平移 ±1（含 45°） | H/V 變多優先；否則挑邊界段縮最短者 | (−H/V, 邊界總長) 字典則下降 |
| 3 | **網格合併** | 相鄰 row 兩兩合併、col 兩兩合併（半平面整體移 1 格、自帶壓縮） | validShift 硬規則全過（不壓點/不新增交叉/拓撲不變）；附帶保證跨距只縮不增、H/V 只增不減 | 網格欄列數每次 −1 |

**循環**＝每輪「端點移動至不動點 → 直線縮減至不動點 → 網格合併至不動點」輪替
（算到不能動才換下一階段／下一輪），某一輪三個演算法都沒有改動才停止。
**234 城全量實測：全部收斂、總計約 5 秒**（單城最慢 NYC 約 0.4 秒，160×160 → 約 31×34）。

**逐步驗證**：浮動面板——「下一步」（目前演算法跑到不動點、再換下一個）、
「下一小步」（單一移動，圖上以**前後比對**標示：舊位置虛線空心圈、
虛線軌跡、新位置橘色實圈，訊息附座標）、「上一步／上一小步」（快照堆疊復原）、
「重設」；三階段 chips 顯示目前工作，一輪全靜止顯示「✔ 收斂完成」。
全用大步跑到收斂的結果**與循環 tab 完全一致**。

直線演算法＝論文①〜⑧＋LLM 對齊共九條鏈（2026-07 使用者裁決：名稱與
`data/thesis` 論文一一對應；先前自創的軸對齊/整數規劃鏈已下架）。②直角爬山
的比較基準（迭代到不動點後）：全網 H/V +42.9%、全數收斂；其餘論文鏈
...
- 視圖組成：`views/` 8 個（原始/旋轉/骨架化/格網化前後 ×2）、`hcviews/` 22 個
  （每變體：格網化後 → Hill Climbing → hc＋論文①〜⑧共 9 條鏈的循環結果，×2 變體）、
  `rwdviews/` 36 個（9 條鏈 ×〔循環結果｜RWD 路網〕×2 變體）。
- **`_fp` 增量重算**：`metro:views` 重跑時指紋沒變的城市直接沿用舊檔，只重算資料變了的城市；
  **改了任何演算法**要把 `buildViews.mjs` 的 `VIEWS_VERSION` +1 強制全量重算
  （版本註解就是演算法變更日誌，目前 36；RWD 佈線器另有 `RWD_ROUTER_REV` 修訂字串）。
...
| LLM 比較（RWD Maps） | 網頁按鈕 → `/llm-compare/run` | skill `route-llm-compare`：唯讀評審——一次比較原始與旋轉變體的論文①〜⑧八條鏈/LLM 對齊（最多 18 個結果），依方正/直線/轉折/平衡/硬失敗選出全體最佳、原始最佳、旋轉最佳並逐一列優缺點 → 寫 `llmcompares/` |
...
│                                #   straightenCells（下游預計算 JSON）、headlessRun（LLM 串流）
...
| # | 文獻 | 對應 |
|---|---|---|
| 1 | Stroke-based schematic maps | **直線鏈「①筆畫法」**（`route-stroke-align`）＋骨架化思路對照 |
| 2 | **Stott et al. 2011**, Automatic Metro Map Layout Using Multicriteria Optimization (TVCG) | **Stage 3 爬山法主依據**＋**直線鏈「②直角爬山」**（`route-rect-polish`，|sin 2θ| 直角變體） |
| 3 | Nöllenburg & Wolff, Mixed-Integer Programming | **直線鏈「③MILP規劃」**（`route-milp-align`） |
| 4 | 力導向自動視覺化 | **直線鏈「④力導向」**（`route-force-align`） |
| 5 | Focus+Context Metro Maps | **直線鏈「⑤最小平方」**（`route-lsq-align`）＋流量權重／概括化思路 |
| 6 | Bast et al., Octilinear Grid Graphs | **直線鏈「⑥八向格網」**（`route-octi-align`；目標最接近三步鏈的「格子越少越好」） |
| 7 | Merrick & Gudmundsson, Path Simplification | **直線鏈「⑦路徑簡化」**（`route-path-align`） |
| 8 | SAT-based Octolinear Layouts | **直線鏈「⑧SAT規劃」**（`route-sat-align`；與 #3 同模型換求解器） |
| 9 | Batik et al. 2022, Shape-Guided Mixed Metro Map Layout (Pacific Graphics) | **Shape-Guided**（`route-shape-align`；掛在①〜⑧循環之後，非直線鏈⑨） |

編號 1–8 的八篇（2026-07 使用者裁決）都以「整數格短距離後處理」形式實作成
Straighten 的**論文直線鏈**（`src/stores/paperAlign.js`，總機構 skill
`route-paper-align`；直線演算法的 tab 名帶論文圈號①〜⑧、與 `data/thesis`
的演算法說明一一對應），加上 LLM 對齊共九條鏈，全部共用同一套硬規則與全部
下游功能。（先前自創的軸對齊/整數規劃鏈因不對應論文已下架。）
