...
- **分社邊界退化已解**：琵琶湖線京都端曾被誤分 JR東海——`trackOrder` 逐元件串接修好後（§3），該線完整
  軌道在 JR西日本 檔內自己串起，23 站全在 west，不再跨社。八ッ場壩 2014 淹沒的舊線殘站（雁ヶ沢/吾妻峡八ッ場，
  距現役軌道 ~980m）拆分後正確剔除（不變式：無軌道之站不輸出；現役 吾妻線 18 站含改線後新 川原湯温泉 完整）。
...
4. **高鐵（high_speed）＝關聯排序但只用停站成員、不 snap**：高鐵**高架橋會壓過下方地方站**（東海道新幹線
   ...
   **一線一 class（多數決，防跨檔重覆）**：CLASSES 拆檔前，先把**每條線名的所有邊統一到其邊多數的 class**。
   否則一般幹線若共用一小段高鐵軌（奥羽本線載 山形新幹線 迷你新幹線、共用 福島–新庄 段標 highspeed），會
   在 `一般國鐵` 出完整自己、又漏 2 站的分身進 `新幹線` 檔＝跨檔重覆。多數決讓 山形新幹線（全 hs）留新幹線、
   温福线/衡柳铁路（中國全 hs 且無一般檔）留高鐵，但把 奥羽本線（2 hs vs 102 一般邊）整條拉回一般國鐵。
