...
description: ②直角爬山（直線演算法論文②——Stott et al. 2011 爬山法的直角變體；tab 名「②直角爬山」）——把爬山法的方向準則從八方向 |sin 4θ| 換成直角 |sin 2θ|（45° 變最貴）在 Hill Climbing 結果上再爬（冷卻/輪數同論文本體：R 8→1、最多 5 輪），並迭代到不動點（上限 20 次）。當使用者要求修改直角爬山、調 rect 模式權重/半徑、或問「直角爬山」tab 的行為時使用。總覽與共用機構見 [[route-hillclimb]]。
---
...
3. **冷卻/輪數同論文本體**（R 從 8 每輪 −1、最低 1，最多 5 輪；`opts.maxMove` / `opts.maxRounds` 可覆寫）。
...
- `buildRectPolish(skeleton, cells, cols, rows, opts)`（src/stores/hillClimb.js）
  = `buildHillClimb(..., { rect: true, weights: { octi: ×3 } })`。
