...
import { RWD_VIEW_ORDER, RWD_VIEW_LABELS } from '../stores/viewGeometry'

// One city's RWD Maps card: a title header + a 4×2 grid of the 8 pre-computed
// RWD views — each row a 縮減網格 source (基本 / 直角爬山 / 軸對齊 / 整數規劃),
// the two columns its 縮減網格 and its RWD 路網 redraw. Geometry is fetched
// lazily (data/metro/rwdviews/<id>.json) when the card scrolls into view.
...
const order = RWD_VIEW_ORDER
const labels = RWD_VIEW_LABELS
let observer = null
...
<template>
  <div ref="root" class="sgrid-card">
    <button class="sgrid-head" :title="`建立 ${entry.cityZh ?? entry.city} RWD Maps 視圖`" @click="emit('pick', entry)">
      <span class="sh-name">
      ...
      <span class="sh-open">建立 RWD Maps ›</span>
    </button>
...
/* 4 rows × 2 columns: 縮減網格 | RWD 路網 per compact source */
.sgrid {
  display: grid;
