...
import ViewSixGrid from './ViewSixGrid.vue'

// A dockview tab showing every city's 6 pre-computed Hill Climbing views as a
// 2×3 card (data/metro/hcviews/, built by scripts/buildViews.mjs). Clicking a
// card imports the city, builds a Map Adjust (D3) view, then a Hill Climbing
// view on top of it — variant (orig/rot) taken from the clicked thumbnail.
...
<template>
  <div class="gallery">
    <div class="gallery-tabs" role="tablist">
      ...
      <span class="gallery-count">{{ tiles.length }} 城市 · 每城 6 視圖</span>
    </div>

    <div class="gallery-body">
      <div v-if="error" class="gallery-status">載入視圖清單失敗：{{ error }}<br />（請先執行 <code>npm run metro:views</code>）</div>
      <div v-else-if="!catalog" class="gallery-status">載入全球 Hill Climbing 視圖清單…</div>
      <div v-else class="tile-grid">
        <ViewSixGrid v-for="s in tiles" :key="s.id" :entry="s" @pick="pick" />
      </div>
    </div>
  </div>
</template>
