...
{ "id":"…", "views": { … "lines":[{d,color}] },
  "hc": { "cellAfter": [[id,c,r], …], "stats": { "hvAfter": 49 } } }`,
    ...
    algorithm: `<p>以「格網化後」為輸入，並排比較 <b>8 個主佈局</b>（①筆畫法／②Hill Climbing／③MILP／④力導向／⑤最小平方／⑥八向格網／⑦路徑簡化／⑧SAT）。左側標籤：<b>②Hill Climbing（往後執行）</b>＝真正的多準則爬山，也是直線演算法／端點移動／RWD 的唯一上游；其餘 7 個標 <b>（僅比較）</b>＝論文鏈 build 直接餵格網，只供觀看、不進下游。tab 名後面的 <b>· 46ms</b> 是該佈局實際算了多久；算過就存起來（含重新整理），只有按「重新計算此城市全部圖層」才清掉重算。</p>
<p><b>② 的計算（Stott et al. 2011，Algorithm 1）</b>：逐頂點掃描以自身為中心、半徑 R 的矩形內所有格點，取「加權多準則適應度」最低且通過硬規則者；再做群集移動；R 從 <b>8</b> 起每輪 −1（最低 1）、最多 <b>5 輪</b>（論文表 4），一輪沒有改善、或一輪後總適應度不再下降就收斂。</p>
<ul><li><b>5 項準則</b>（權重＝論文表 4）：角解析度 30000／邊長 50／平衡邊長 45／線條平直 220／八方向 9250。</li><li><b>4 條硬規則</b>（違反即否決）：邊界（含一格一站）、相對位置（象限不翻轉）、無遮蔽（不疊站/不壓邊/不新增交叉）、邊環繞順序不變。</li><li><b>群集移動</b>（對付局部極小）：§6.1 超長邊群集（以非超長邊做連通分量整體平移）＋§6.2 折彎群集（度 2 且兩鄰段同路線、偏離 u→w 超過 22.5° 的 kink 單獨移動）。</li></ul>
<p><b>與論文不同的地方</b>：<b>標籤的 7 項準則不做</b>——本管線沒有標籤佈局層（站名是 Style 開關，不是佈局的一級公民）；<b>§6.3 二分割群集不做</b>（論文本身列為選配）。其餘準則、權重、硬規則、冷卻與收斂條件都照論文。</p>`,
  },
  straighten: {
...
<p><b>忠實度</b>：每條鏈的準則、成本常數、權重、流程順序都照對應論文的說明檔實作
（②冷卻 8→1／5 輪；①式 3 排序＋4 主方向 H/V 優先、45° 為備援；③⑧ 每段 3 候選方向、
權重 (λ1,λ2)=(3,2)、最短邊長＝該段吞掉的白點數＋1；④ 論文原力式與 c_m=0.1／b=30／
α=1／β=0.5 ＋ PrEd 8 區域移動上限；⑤ w_o=10／w_g=0.05；⑥ c_45=2／c_90=1.5／c_135=1／
c_h=1／c_m=0.5 與 ldeg dangling 排程；⑦ 最大彎角 90°＋最小 link 長）。以下是<b>刻意保留的差異</b>：</p>
<ul><li><b>共通</b>：原論文都是「全圖佈局器」，這裡是接在②之後的<b>短距離後處理</b>——
位移夾 ±2 格，且結果一律要過②的 4 條硬規則與「H/V+45° 段數不變差」的接受準則才套用
（論文各自的拓撲保證機制由這一層取代）。白點（中間站）不參與計算，等彩色點算完才沿新段
平均放回——對應各論文的 deg-2 收縮＋等距回插。</li>
<li><b>③MILP</b>：瀏覽器沒有 MIP 求解器，改用生成樹 DP＋feedback 段枚舉<b>精確求解方向
指派子問題</b>；S3（總長最小化）交給下游「縮減網格」、H4（平面性）交給硬規則。</li>
<li><b>⑧SAT</b>：與③同模型，MaxSAT 求解器換成 DPLL 分支定界；節點預算 60000 ＝論文的
timeout（超限的元件退回原方向）。</li>
<li><b>⑥八向格網</b>：論文在格網圖上跑多彎 Dijkstra 路由，但本管線「彩色點之間就是一條
直段」無法表示多彎路徑，因此只取其<b>候選集＋成本模型＋貪婪定案順序</b>做節點指派；
STEP 5 的 local search 由「迭代到不動點」取代。多彎路由是 RWD 畫線器的職責。</li>
<li><b>⑦路徑簡化</b>：link boundary 取可行 offset 區間中點、不是完整上包絡，因此
<b>「最少 link 數」的理論保證會弱化</b>；頂點是垂直投影到 link 上（捷運站一律保留），
不是論文的 ComputePath 反推。忠實度是八條裡最低的一條。</li>
<li><b>④力導向</b>：GEM 隨機初始階段不跑（輸入已是爬山後的佈局，論文允許以既有佈局
起始，也讓結果<b>可重現</b>）；另加 GEM 的溫度上限防止格空間近距離斥力數值溢位。</li>
<li><b>⑤最小平方</b>：只跑階段二（八方向化）——階段一的平滑變形與 coarse-to-fine 對
已格網化的輸入沒有作用；共軛梯度換成 Gauss–Seidel（同一個最小平方解）。</li>
<li><b>①筆畫法</b>：拓撲一致性不用論文的「點在多邊形→推點」修復，改由硬規則擋下會翻轉
拓撲的投影（效果等價於「移不動就留在原地」）。</li></ul>`,
  },
  'endpoint-move': {
